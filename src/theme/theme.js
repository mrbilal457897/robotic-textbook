 ...
 */
export function applyCSSVariables(element, theme = defaultTheme) {
...
export function responsive(styles) {
    ...
    if (xxl) {
        css += ` @media (min-width: ${tokens.breakpoints['2xl']}) { ${Object.entries(xxl)
            .map(([key, value]) => `${camelToKebab(key)}: ${value};`)
            .join(' ')} }`;
    }
    return css;
 ...
 */
export function glassmorphism(blur = '12px', opacity = 0.7) {
    return {
 ...
 */
export function glow(color = 'primary', intensity = 0.5) {
    const colorMap = {
 ...
 */
export function transition(properties = ['all'], duration = 'base', timing = 'easeInOut') {
    return {
        transition: properties
            .map((prop) => `${prop} ${tokens.transitions.duration[duration]} ${tokens.transitions.timing[timing]}`)
            .join(', '),
    };
}
/**
 ...
 */
export function card(elevated = false) {
    return {
        ...glassmorphism(),
