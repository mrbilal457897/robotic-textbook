...
export function ToneSelector({ tone, onChange, disabled = false, className }: ToneSelectorProps) {
  const [isOpen, setIsOpen] = React.useState(false);
  const selectedTone = TONES.find(t => t.value === tone);

  return (
    <div className={cn('relative flex flex-col gap-2', className)}>
      <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Response Tone</label>
      ...
      >
        <div className="flex items-center gap-2">
          {selectedTone && <span className="text-lg">{selectedTone.icon}</span>}
          ...
          <div className="absolute top-full z-20 mt-2 w-full rounded-lg border border-gray-300 bg-white shadow-lg dark:border-gray-700 dark:bg-gray-900">
            {TONES.map(toneOption => {
              ...
              return (
                ...
                >
                  <span className="mt-0.5 text-lg">{toneOption.icon}</span>
